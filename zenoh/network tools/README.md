...
```bash
# 1. On machine A (router)
zenohd --listen tcp/0.0.0.0:7447

# 2. Verify it's listening
sudo ss -tulnp | grep 7447

# 3. On machine B, test TCP connectivity
nc -zv <MACHINE_A_IP> 7447

# 4. If nc works, start subscriber on machine A
python3 examples/z_sub.py -m client -e tcp/127.0.0.1:7447

# 5. Start publisher on machine B
python3 examples/z_pub.py -m client -e tcp/<MACHINE_A_IP>:7447
```

## Useful Python Network Testing

### Simple HTTP server (test if any TCP works)
```bash
# On server machine
...
This tests basic TCP connectivity without Zenoh.